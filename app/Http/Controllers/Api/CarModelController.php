...
class CarModelController extends Controller
{
    ...
    public function update(UpdateCarModelRequest $request, $id)
    {
        ...
        try {
                ...
                $data['image_start_year'] = $this->updateImage($model->image_start_year);
                $data['image_end_year'] = $this->updateImage($model->image_end_year);
    ...
    public function destroy($id)
    {
        ...
        }catch(ModelNotFoundException $e){return $this->success(200,[],__("No data found"));}
    }
    //for website 
    ...
    public  function updateImage($imageName)
    {
        if (request()->hasFile('image_start_year')) {
            ...
            return uploadImage(request()->file('image_start_year'), "Models");
        } else if(request()->hasFile('image_end_year')) {
            deleteImage($imageName, "Models");
            return uploadImage(request()->file('image_end_year'), "Models");
